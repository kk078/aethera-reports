-- Aethera Reports hosted client portal (Phase 3 chunk F) — migration 0001
-- (initial schema). Applied via: wrangler d1 migrations apply — see
-- docs/portal.md. Kept statement-identical to ../schema.sql, which is
-- the human-readable reference copy of this same schema.
--
-- Never store patient-level data here — a snapshots.report_json value is
-- always a ClientReport (aggregate KPIs/financials/aging only), enforced
-- by validating/stripping against src/shared/domain.ts's
-- clientReportSchema on every publish.
